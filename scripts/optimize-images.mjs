...
async function optimizeImage(inputPath, outputBasePath, maxWidth, quality) {
  ...
  try {
    ...
    console.log(`  WebP: ${(webpStats.size / 1024).toFixed(0)}KB (${((1 - webpStats.size / inputStats.size) * 100).toFixed(0)}% smaller)`);
    console.log(`  PNG: ${(pngStats.size / 1024).toFixed(0)}KB (${((1 - pngStats.size / inputStats.size) * 100).toFixed(0)}% smaller)`);
